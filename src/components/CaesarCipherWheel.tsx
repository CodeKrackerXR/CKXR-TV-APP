   ...
   <div className="min-h-screen w-full relative bg-black overflow-x-hidden flex flex-col font-sans text-white">
     ...
     {promptStep === 'SPEED_LINK' && (
       <div className="fixed inset-0 z-[300] flex items-center justify-center p-6 bg-black/95 backdrop-blur-2xl animate-[fadeIn_0.3s_ease-out]">
         <div className="w-full max-w-xl bg-vault-panel border-2 border-vault-gold p-10 rounded-3xl text-center relative overflow-hidden shadow-[0_0_50px_rgba(212,175,55,0.2)]">
     ...
     <div className="relative z-10 container mx-auto px-4 flex-1 flex flex-col lg:flex-row items-center justify-center gap-12 py-8">
       {/* Mobile/Tablet Portrait Headline */}
       <div className="lg:hidden w-full flex flex-col items-center mb-4">
         {isTimerActive && promptStep === 'NONE' && (
           <div className="bg-black/85 border border-vault-gold/30 px-4 py-2 rounded-xl shadow-2xl backdrop-blur-2xl w-fit flex flex-col items-center overflow-hidden relative mb-4">
             <span className="text-[8px] font-display text-white/80 uppercase tracking-[0.2em] mb-0.5 font-black">Caesar Timer</span>
             <div className="flex justify-center items-baseline">
               <span className="text-lg md:text-xl font-mono font-light text-vault-gold tabular-nums tracking-normal drop-shadow-[0_0_8px_rgba(212,175,55,0.4)]">
         ...
         <h2 className={`text-xl md:text-3xl font-display font-black uppercase tracking-widest text-center ${!hasInteractedWithWheel ? 'animate-blink' : ''} ${shift === expectedShift ? 'text-vault-gold' : 'text-white'}`}>
           {shift === expectedShift ? (
             <>
               Now pick the <span className="text-white">white</span> letters to crack the code
             </>
           ) : (
             <>
               ROTATE <span className="text-vault-gold">YELLOW</span> LETTERS TO MAKE <span className="text-vault-gold">{mappingLetter}={huntAnswers[8] || "0"}{huntAnswers[9] || "0"}</span>
             </>
           )}
         </h2>
       </div>
       ...
       <div className="w-full max-w-2xl flex flex-col gap-6">
         ...
         <div className="hidden lg:flex flex-col items-start">
           ...
           <h2 className={`text-xl md:text-3xl font-display font-black uppercase tracking-widest text-left ${!hasInteractedWithWheel ? 'animate-blink' : ''} ${shift === expectedShift ? 'text-vault-gold' : 'text-white'}`}>
             {shift === expectedShift ? (
               <>
                 Now pick the <span className="text-white">white</span> letters to crack the code
               </>
         ...
         <div className="w-full bg-vault-panel/90 backdrop-blur-xl border border-vault-gold/30 p-6 md:p-10 rounded-2xl shadow-2xl relative">
