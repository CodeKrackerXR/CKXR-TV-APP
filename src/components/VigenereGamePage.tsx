...
export const VigenereGamePage: React.FC<VigenereGamePageProps> = ({ onReturnToEncoder, initialCode, initialKey, targetText, onPostResults, youtuber }) => {
 ...
 const keystreamInputRef = useRef<HTMLInputElement>(null);
 ...
 const handleCellClick = (rowIndex: number, colIndex: number, e: React.MouseEvent) => {
   ...
   if (selectedRowIndex === rowIndex) {
     ...
     // The column letter is the decrypted letter
 ...
 const handleAccept = (e: React.MouseEvent) => {
   e.stopPropagation();
   if (isFinished) return;
   const nextChar = popupData.cracked;
   setCrackedLetters(prev => prev + nextChar);
   setShowPopup(false);
   setSelectedRowIndex(null);
   setSelectedColIndex(null);
 };

 const handleKeystreamChange = (val: string) => {
   if (isFinished) return;
   const upper = val.toUpperCase().replace(/[^A-Z]/g, '');
   const maxLen = initialCode?.length || 0;
   setUserKeystream(upper.slice(0, maxLen));
 };

 const nextPairIndex = crackedLetters.length;
 const nextKeystreamLetter = userKeystream[nextPairIndex] || '-';
 ...
 return (
   <div className="min-h-screen w-full relative bg-black overflow-x-hidden flex flex-col font-sans text-white pb-20" onClick={() => setShowPopup(false)}>
     ...
     <div className="relative z-10 container mx-auto px-4 py-8 max-w-6xl">
           ...
           <div className="bg-vault-panel/80 border border-vault-gold/30 rounded-2xl p-6 shadow-2xl backdrop-blur-xl">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                 <div className="space-y-4">
                   <div>
                     <label className="block text-[10px] text-green-500 uppercase tracking-widest font-black mb-2">Type in Key Word</label>
                     <input
                       ref={keywordInputRef}
                       type="text" 
                       value={userKeyword}
                       ...
                       onKeyDown={(e) => {
                         if (e.key === 'Enter') keystreamInputRef.current?.focus();
                       }}
                       disabled={isFinished}
                   ...
                   <div>
                     <label className="block text-[10px] text-blue-500 uppercase tracking-widest font-black mb-2">Type in Key Stream</label>
                     <input
                       ref={keystreamInputRef}
                       type="text"
                       value={userKeystream}
                       onChange={(e) => handleKeystreamChange(e.target.value)}
                       disabled={isFinished}
                       className="w-full bg-black/60 border border-blue-500/30 rounded-lg p-3 font-display font-bold text-blue-500 focus:outline-none focus:border-blue-500 tracking-widest disabled:opacity-50 disabled:cursor-not-allowed"
                       placeholder="REPEAT KEY WORD TO MATCH LENGTH"
                     />
                   </div>
                 </div>
                 ...
                 <div className="flex flex-col justify-center items-center bg-black/40 rounded-xl p-6 border border-white/5 overflow-x-auto">
                   <div className="min-w-max flex flex-col items-center">
                      <label className="block text-[10px] text-blue-500/60 uppercase tracking-widest font-black mb-2">Keystream</label>
                      <div className="flex justify-center gap-1 mb-2 border-b border-white/10 pb-2 w-full">
                        {(userKeystream || '---').split('').map((l, i) => (
       ...
       {/* Ciphered Text Display - Centered and Yellow above Grid */}
       ...
       {showPopup && (
         <div
           className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[200] animate-[fadeIn_0.3s_ease-out]"
           ...
           <div className="bg-vault-panel/95 border-2 border-vault-gold p-8 rounded-2xl shadow-[0_0_60px_rgba(212,175,55,0.5)] backdrop-blur-2xl min-w-[380px] text-center relative overflow-hidden">
              <div className="space-y-6 font-display font-black uppercase tracking-widest">
                <p className="text-white text-xl">
                  KEYWORD LETTER: <span className="text-[#ebc805]/90">[{popupData.keyword}]</span>
                </p>
                <p className="text-white text-xl">
              ...
              <div className="flex gap-4 mt-8">
                 <button
                   ...
                   className="flex-1 bg-green-600 text-white py-4 font-display font-black uppercase tracking-widest text-sm hover:bg-green-500 transition-all rounded-lg shadow-lg active:scale-95"
                 >
                   Accept
                 </button>
                 <button
                   onClick={() => setShowPopup(false)}
                   className="flex-1 bg-red-600 text-white py-4 font-display font-black uppercase tracking-widest text-sm hover:bg-red-500 transition-all rounded-lg shadow-lg active:scale-95"
                 >
                   Cancel
                 </button>
              </div>
       ...
       {/* Cracked Letters Display at Bottom */}
         ...
         <div className="p-6 md:p-10 flex flex-col items-center justify-center gap-6">
           <div className="relative w-full">
             ...
             {crackedLetters && !isFinished && (
               <button
                 onClick={() => setCrackedLetters(prev => prev.slice(0, -1))}
                 className="absolute right-4 top-1/2 -translate-y-1/2 text-white/40 hover:text-vault-alert transition-colors"
                 title="Clear Last Letter"
               >
           ...
           <div className="flex justify-center mt-4">
             {!isFinished ? (
               ...
               >
                 <div className="bg-vault-gold text-black px-10 py-4 rounded-full font-display font-black text-sm uppercase tracking-widest flex items-center gap-2 shadow-[0_0_30px_rgba(212,175,55,0.5)] border-2 border-white/20">
