...
export const CaesarGamePage: React.FC<CaesarGamePageProps> = ({
  onBack,
  onReturnToEncoder,
  onNavigateToTheHunt,
  ...
  const handleVerifySubmission = async (overrideValue?: string) => {
    ...
    try {
      ...
      // Try fetching from Firestore first as primary source of truth for "The Hunt"
      try {
      ...
      // If Firestore failed or is empty, fallback to local decoding of initialCode using expectedShift
      if (!correctWord) {
      ...
      if (valueToVerify === correctWord && correctWord !== "") {
        ...
        await setDoc(
          userRef,
          {
            "Level 1": { caesarCipherTime: finalFormattedTime },
        ...
        setIsTimerRunning(true); // Restart timer on failure
      }
    } catch (err: any) {
              ...
              <div className="pt-8">
                <button
                  onClick={() => {
                    if (onPostResults) {
                      onPostResults({
                        sponsorKey: `${mappingLetter}=${ALPHABET[expectedShift]}`,
      ...
      {showRotationError && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-6 bg-black/95 backdrop-blur-md">
          <div className="w-full max-w-xl bg-zinc-900 border-2 border-red-500/50 p-10 rounded-3xl text-center shadow-[0_0_100px_rgba(239,68,68,0.2)]">
            ...
            <div className="mb-8 text-7xl font-black text-white">
              {mappingLetter}=
              <span className="text-[#D4AF37]">{ALPHABET[expectedShift]}</span>
            </div>
            <p className="text-[#D4AF37] text-lg mb-12 italic uppercase font-black">
              "You must rotate the center yellow wheel first to set the shift
              key"
            </p>
            <button
          ...
          <div className="w-full max-w-lg bg-zinc-900 border-2 border-[#D4AF37]/50 p-10 rounded-3xl text-center shadow-[0_0_100px_rgba(212,175,55,0.2)]">
            ...
            <div className="flex flex-col gap-6 mb-10 border-y border-white/5 py-8">
              ...
              <div className="flex justify-between items-center px-4">
                ...
                <span className="text-4xl font-black text-white">
                  {mappingLetter}=
                  <span className="text-[#22c55e]">{ALPHABET[shift]}</span>
                </span>
              </div>
            </div>
            <p className="text-[#D4AF37] text-md mb-12 italic font-black uppercase">
              "Your value doesn't match the shift key. Do you want to move
              forward?"
            </p>
        ...
        <div className="w-full max-[500px] md:max-w-[700px] flex flex-col items-center">
          ...
          >
            ...
            >
              ...
              >
                {mappingLetter} ={" "}
                <tspan fill="#d4af37">{ALPHABET[(ALPHABET.indexOf(mappingLetter) + shift) % 26]}</tspan>
              </text>
              <text
                x="50"
                y="42"
        ...
        <div className="w-full max-w-xl space-y-8">
          <div className="mb-6">
            <h2 className="text-xl md:text-3xl font-black uppercase tracking-[0.2em] leading-tight text-center lg:text-left">
              {isSolved ? (
                <span className="text-[#22c55e]">Congratulations! Code Cracked</span>
              ) : shift === expectedShift ? (
                <span className="text-[#22c55e] font-black uppercase">
                  Now pick the <span className="text-white">White Letters</span> to crack the code
                </span>
              ) : (
                <span className="text-[#22c55e] font-black uppercase">
                  ROTATE <span className="text-[#D4AF37]">YELLOW</span> LETTERS TO
                  MAKE{" "}
                  <span className="text-[#D4AF37]">
          ...
          <div className="bg-[#121212] border border-[#D4AF37]/20 p-8 md:p-12 rounded-3xl shadow-2xl relative overflow-hidden group">
            <div className="space-y-10 relative z-10">
              <div className="space-y-4">
                ...
                <div className="w-full bg-black/60 border border-white/5 rounded-xl p-8 font-black text-2xl text-[#D4AF37] uppercase text-center tracking-[0.2em] shadow-inner">
                  {cipherInput || "------"}
                </div>
              </div>
              ...
              <div className="pt-6 space-y-4">
                ...
                >
                  {isVerifying ? "VERIFYING..." : isSolved ? "CRACKED" : "Crack Code"}
                </button>

                <div className="flex justify-between items-center text-[10px] font-black text-white/20 uppercase tracking-widest pt-4">
                  <span>Node: {CREATOR_DOC_ID}</span>
                  <span>Status: Encryption Bypassed</span>
                </div>
              </div>
            </div>
          </div>
