...
export const AtlasCipherPage: React.FC<AtlasCipherPageProps> = ({ onBack, youtuber, onPlay }) => {
  ...
  // Convert result string to numbers for display
  const numericResult = useMemo(() => {
    return result.split('').map(letterToNumber).join('-');
  }, [result]);
            ...
            <div className="space-y-8">
              ...
              <div>
                 <label className="flex justify-between items-center text-[10px] font-black text-[#D4AF37]/50 uppercase tracking-[0.2em] mb-3">
                   ...
                   <span className="text-[#D4AF37] text-sm">{shift + 1}</span>
                 </label>
                 <input
                  type="range"
                  min="0"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
              ...
              <div className="bg-black/20 rounded-2xl p-6 border border-white/5 flex items-center">
                 <p className="text-xs text-white/50 font-bold uppercase leading-relaxed italic border-l-4 border-[#D4AF37] pl-6 py-2">
                   "The shift value of {shift + 1} determines the character offset. Each letter is converted to its respective position in the alphabet plus the key value."
                 </p>
              </div>
            </div>
